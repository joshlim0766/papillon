...
`̂P_N`의 정의는 **동일 `(x, c, θ)`로 N회 호출**한 표본을 전제한다. 그러나 자연 운영 환경에서 사용자는 동일 `(x, c, θ)` 호출을 거의 발생시키지 않는다 — 동일한 의도라도 매번 표현·맥락·시점이 달라지므로 byte-equals 기준으로는 **N은 거의 항상 1**이다.

따라서 `̂P_N`은:

- **인공적 반복 호출 실험에서는 추정 가능**: 의도적으로 동일 `(x, c, θ)`를 N회 보내는 stability runs·fixture 검증 등.
- **자연 운영 데이터에서는 직접 추정 불가**: byte-equals 기준 N=1.

자연 데이터로 `̂P_N` 류 분포를 정의·추정하려면 **input 측에도 equivalence relation `~_input`이 필요**하다. `(x, c, θ) ~_input (x', c', θ')`인 호출들을 같은 호출로 묶어 equivalence class 위에서 N을 모은다. 이 `~_input`의 정의는 **L1(Prompt 공간의 구조)의 주제**이며, L0에서는 다루지 않는다.

<rationale>
Output 쪽에 투영 `π`를 도입한 것과 대칭적으로, input 쪽에도 equivalence가 필요하다. 그러나 input equivalence는 prompt 공간의 유사도·변환 불변성을 다루는 L1의 본격 주제이므로, L0은 "필요성"만 명시하고 정의는 L1에 위임한다. L0 독자는 "̂P_N을 자연 데이터에서 어떻게 추정하나"라는 질문에 막히지 않고 L1으로 자연스럽게 넘어갈 수 있다.
</rationale>
