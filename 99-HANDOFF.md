...
## 현재 작업 위치
`01-Planning/` 전체 — 스킬 변환 사이클 진입

## 다음 작업: 스킬 변환 사이클

설계서 → 스킬 변환 시 `<rationale>` 태깅을 동시 수행. 각 스킬 완성 후 실험 데이터 축적.

| 순서 | 대상 | 설계서 상태 | 스킬 상태 | 태깅 | 비고 |
|---|---|---|---|---|---|
| 1 | inquisition | 완성 | 설치됨 (`inquisition.md`) | **완료** | 태깅 14건, 설계서 수정 4건, 스킬 불일치 4건 수정 |
| 2 | wtth | 완성 | 미작성 (설계서 보고 수동 실행 수준) | **미태깅** | 설계서 태깅 + 정식 스킬 작성 |
| 3 | shackled | 완성 | 미작성 | **미태깅** | 설계서 태깅 + 스킬 작성. 여기서부터 실험 가능 |
| 4 | papillon | 완성 | 미작성 | **미태깅** | 설계서 태깅 + 오케스트레이터 스킬. 풀사이클 준비 완료 |

**실험 소재:** 별도 오퍼스 세션에서 인터뷰하여 만든 설계 문서 있음. 인퀴지션에 던져서 파이프라인 실험 가능.

**Issue-02 검증:** 스킬 변환 사이클 + 파이프라인 실험 과정에서 #2(컨텍스트 윈도우), #3(사람 개입), #4(모드별 상세) 자연 검증.
...
## 다음 작업: wtth 스킬 변환 사이클

inquisition과 동일한 패턴: 설계서 `<rationale>` 태깅 → 스킬 작성 → 불일치 수정. SPEC/RAT 태깅 규칙은 `02-common-spec.md §12`에 확정됨.
...
| # | 심각도 | 제목 | 솔루션 핵심 | 적용 시점 |
|---|---|---|---|---|
| 1 | 높음 | 설계서 vs 프롬프트 스펙 혼재 | XML 태그로 구조적 격리 | inquisition 적용 완료, 나머지 순차 적용 |
...
| 4 | 중간 | wtth 모드별 상세 부족 | 모드별 스킵/변형/추가 규칙 명시 | wtth 스킬 작성 시 |
