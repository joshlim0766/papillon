...
## 현재 작업 위치
**회사 운영 실측 + 5건 후속 통찰 박제 완료** (2026-04-25, 집). 다음 세션 = 회사. 회사 잔여 4건(통계수집 워커·페이로드 발송·대사배치·통계 롤업) = **변형 C(Reverse-then-Forward) 시범 4건** = 빠삐용-ASEWS 접면 1호 영역. 단일 스펙 라이프사이클 (b) 인덱스+분할 형태로 1건 이상 시도 + 측정 17항목 동시 수행 권장.

**중요 1 — 동기화 보장**: 메모리(`~/.claude/projects/.../memory/`)는 머신 종속. 회사·집 별개 맥북이면 회사 세션에서 안 보임. 따라서 본 HANDOFF에 **메모리 핵심 압축 인라인** (아래 §후속 통찰 5건 참조). 메모리 본문 상세는 집 머신만.

**중요 2 — 스킬**: wtth 스킬 (`~/.claude/commands/papillon/wtth.md`) §2.0 심각도 5단계 박제도 레포 밖 편집. 회사 머신 동기화 여부 spot check 필요.
...
## 후속 통찰: 2026-04-25 (집) — 5건 추가 논점 + 빠삐용 정체성 진화 시그널

같은 날 후속 대화에서 빠삐용 설계 메타에 닿는 5건 추가 논점 박제. 회사 맥북 동기화 보장 위해 **압축 인라인**.

### 통찰 1 — 검토 부하 비대칭 (메모리: `project_review_capacity_asymmetry.md`)

AI 협업이 산출물 폭증(한 지라 티켓당 5~10+ 문서) → 사람 검토 여력 한정. **N=1 양호 시그널이 (A)품질 vs (B)여력 한계 형식 통과 구분 불가**. 빠삐용 자체가 인플레이션 가속기 — 회사에서 자동 파이프라인 우회된 한 축이 이것일 가능성. 빠삐용 부분 답 산재(Gate/Checkpoint/FYI 3등급·wtth 수렴·fixture runner·Sonnet/Opus 분담·HITL escalation) 단일 프레임 부재. **Issue-09 (가칭) "검토 부하 회계 + 산출물 인플레이션 대응" 후보**, N≥3 누적 후 결정.

### 통찰 2 — 단일 스펙 라이프사이클 + 3변형 (메모리: `project_unified_spec_lifecycle.md`)

산출물 폭증의 자연 해결책: **수렴 시점에 단일 SSOT 응축**. 단, **단일 SSOT ≠ 단일 파일**. Claude long-context 약화 실증(주인님 경험)으로 **다중 파일 + 인덱스** 패턴 필수.

**3변형**:
| 변형 | 케이스 | 패턴 |
|---|---|---|
| A. Forward | 순수 신규 (Kotlin 5월) | 인터뷰→설계→TDD→수렴 SSOT 응축 |
| B. Reverse | 순수 레거시 개선 | 코드+KB→Reverse Spec→점진 누적 SSOT |
| **C. Reverse-then-Forward** | 레거시 대체용 신규 | 레거시 KB→Reverse 호환 요구→Forward 신규 설계 |

**분할 구조 후보** (관심사 × 수명):
```
spec/
  00-index.md       — 진입점
  01-context.md     — 인터뷰 흡수
  02-geis.md        — 도메인 금기 [락]
  03-contract.md    — Contract 4축 [락]
  04-architecture.md— 설계 (SPEC+RAT) [변동]
    04a-as-is.md    — 변형 B/C: 기존 동작
    04b-to-be.md    — 변형 B/C: 개선 후
    04c-diff.md     — 변형 B/C: 변경 매니페스트
  05-test.md        — TDD 시나리오 [락]
  06-operations.md  — 운영 (해당 시)
  07-trace.md       — RDR/ADR 링크 (참조)
```

**파일 크기 상한 ≈ Claude 1회 호출 안정 흡수 컨텍스트**. 모델 변경 시 정책 재평가.

### 통찰 3 — ASEWS-레거시 접면 + 어제 결정 1 세분화 (메모리: `project_asews_legacy_interface.md`)

회사 잔여 4건이 **Kotlin 5월 론칭과 같은 레포의 변형 C 작업**으로 식별. 어제(2026-04-22) 결정 1("빠삐용-ASEWS 통합 없음")과 **충돌이 아닌 보완**:

- **결정 1 그대로 유지** (스킬 자동 통합 없음, 스킬 수정 보류)
- **세분화**: "결정적 3순간" 중 (i) 접면 설계 결정 → 변형 C에서 **(i') Reverse Spec 추출**로 일반화. 빈도 갱신: **변형 C 작업 단위마다 1회 이상 자연 발생** (어제 "일회성" 인상 정정)
- 회사 잔여 4건 = **빠삐용-ASEWS 접면 1호 시범**
- ASEWS 1호 파일럿 후보가 어제 박제(Kotlin 5월 후) **외에 변형 C 회사 작업도 동급 후보** 시사

### 통찰 4 — 빠삐용 정체성 진화 시그널 (메모리: `project_papillon_identity_evolution.md`)

**팀원 자발 모방 사례 발견** — work-plan 레포 + 지라 티켓 단위 디렉토리 + AI 인터뷰·설계 산출물 git 박제 + 지라 티켓 정리 기록. 강제 도입 아닌 자발 모방은 강한 가치 시그널.

**자연 확산 깊이 6단계** (후속 박제):
| 깊이 | 시그널 | 관찰 |
|---|---|---|
| 1 | 형식 모방 (work-plan + 지라 디렉토리 + md 박제) | ✅ |
| 2 | AI 설계를 md로 작성 (산출물 형식 자발 채택) | ✅ |
| 3 | md 사이즈 적정성 의문 (같은 논점 자력 도달) | ✅ |
| 4 | "문서가 커지면 클로드가 놓친다" pain point 자연 표현 | ✅ |
| 5 | 주인님 조언("단일책임·단일관심사 쪼개기") = 빠삐용 분할 정책 4축 중 2번(관심사 단위) 정확 일치 — **자력 재발견** | ✅ |
| 6 | 팀원이 자력 답 도달 (분할 적용 후 결과) | 관찰 보류 |

**Claude long-context 약화 N 확장**: 어제 박제 N=1(주인님 단독) → **N=다수 (주인님 + 팀원)**. 운영 제약 굳히기 충분.

**두 층위 분리**:
| 층위 | 본질 | 의존성 |
|---|---|---|
| **1. 업무 방식 스펙** | 사람·AI 협업 규약·산출물·라이프사이클 | 없음 — 사람 자력 가능 |
| **2. 자동화 실행** | 스킬·페르소나·수렴·shackled·인퀴지션 | Claude Code + **종량제 API 필수** |

- 정액제 구독 → 빠삐용 층위 2 운영 불가. 종량제 API 필수
- 팀원 모방 = 층위 1만 가져갔을 가능성 → 층위 2 진입장벽 시사
- **박제 정책 결정**: 레포 = 업무 방식 SSOT 정본. 메모리·스킬 = 작업자 컨텍스트 보조. **핵심 박제는 반드시 레포에**
- 정체성 변경 결정은 N + 외부 도입 + v0.3.0 누적 후

### 통찰 5 — 빠삐용 자기 메타 시그널 (메모리: `project_papillon_self_meta.md`)

빠삐용이 박제 중인 논점을 **빠삐용 박제 시스템 자체 + 외부 사용자가 실증으로 겪는 케이스 누적**.

**사례 1 (2026-04-25)**: 메모리-HANDOFF 동기화 = 검토 부하 비대칭 + 다중 머신 SSOT 일관성의 실증. 메모리는 머신 종속, 회사·집 별개 맥북이면 박제 핵심이 회사에서 안 보임. 통찰 4 두 층위 분리와 자연 정합 → 본 HANDOFF에 압축 인라인 (옵션 B). 후속 옵션 C(레포 내 `meta/memory/` + 동기화 스크립트) Issue 후보.

**사례 2 (2026-04-25 후속)**: 외부 사용자(팀원) 자력 도달 + 주인님 조언이 빠삐용 박제와 자연 수렴. 팀원이 "Claude 놓침" pain point 자연 표현 + 주인님이 빠삐용 메모리 미열람 상태에서 권한 답("단일책임/관심사 쪼개기")이 빠삐용 분할 정책 4축 중 2번(관심사 단위) 정확 일치. = **자기 일관성 강한 검증**. 빠삐용 답이 추상이 아닌 사람·AI 협업의 자연 패턴일 가능성 시사. 외부 도입 가이드 자연 트리거 발견 (pain point → 빠삐용 답 진입).
...
**목적**: 기존 레거시 통계 시스템 대체 (같은 레포 = Kotlin 5월 론칭 레포)

### 다음 회사 세션 액션 (갱신)

**측정 21항목** (1차 박제 5축 + 검토 부하 4축 + 변형 C 4축 + 팀원 모방 4축 + 팀원 분할 적용 관찰 4축):

기본 5축:
1. tdd-ready-spec v0.2.0 신호 9개 ↔ Contract 4축 매핑
2. anti-pattern이 Geis로 작동한 흔적
3. 주인님 지적 종류 분포 (typo/Contract 보강/Geis 발견) — AI 단독 vs prior 메꿈 분리
4. 사람 개입 없이 통과한 항목 비율
5. Opus 산출물의 SPEC/RAT 자연 준수 여부

검토 부하 4축:
6. 산출물 페이지/줄 수 ↔ 검토 시간 비율
7. 검토 깊이 분포 (전수 정독/핵심만/스캔/스킵)
8. "양호 통과" 자기 진단 (A품질 vs B여력 한계)
9. 지적이 산출물 어느 위치에서 자주 나왔나

변형 C 4축:
10. Reverse 단계 실제 수행 여부 자기 인지
11. Reverse 단계 KB 활용 빈도
12. 레거시 호환 Contract와 신규 설계 분리도
13. 호환 회귀 시뮬레이션 RED 시나리오 비율

팀원 자발 모방 (가능 시):
14. 팀원 work-plan 패턴 N=? (몇 명, 몇 티켓)
15. 가져간 빠삐용 부분 vs 안 가져간 부분
16. 진입장벽으로 작용한 부분
17. 효용성 자기 진단

팀원 분할 적용 관찰 (주인님 조언 후 — 단일책임/관심사 쪼개기):
18. 분할 단위 (어떻게 쪼갰는지 — 관심사? 섹션? 수명?)
19. Claude 놓침 빈도 변화 (분할 전 vs 후)
20. 분할 비용 발생 (관리 부담, 파일 간 정합성 비용)
21. pain point 잔존/소멸 ("문서가 커지니 클로드가 놓친다" 표현 재발 여부)

**시범 작성**: 잔여 4건 중 1건 이상을 변형 C 단일 스펙 라이프사이클 (b) 인덱스+분할 형태로 시도. 통찰 2 분할 구조 적용. 측정 21항목 동시 수행.

### 박제 산출물 (2차, 후속)

- 신규 메모리 `project_review_capacity_asymmetry.md`
- 신규 메모리 `project_unified_spec_lifecycle.md`
- 신규 메모리 `project_asews_legacy_interface.md`
- 신규 메모리 `project_papillon_self_meta.md`
- 신규 메모리 `project_papillon_identity_evolution.md`
- 갱신 메모리 `project_company_runtime_pattern.md` — 변형 C 한정 조건 + 측정 21항목 + 팀원 자발 모방 + 팀원 분할 적용 관찰 추가
- 갱신 메모리 `project_contract_geis_direction.md` — 결정 1 세분화 박제
- MEMORY.md 인덱스 5건 추가
- 본 HANDOFF 압축 인라인 + 통합 커밋 + 푸시

### 박제 산출물 (3차, 후속의 후속)

팀원 자력 도달 + 주인님 조언 자연 수렴 시그널 추가 박제:
- 갱신 메모리 `project_unified_spec_lifecycle.md` — Claude 약화 N 확장 + 자기 일관성 검증 추가
- 갱신 메모리 `project_papillon_self_meta.md` — 사례 2 추가 (외부 자력 도달)
- 갱신 메모리 `project_papillon_identity_evolution.md` — 자연 확산 깊이 6단계 + 외부 도입 트리거
- 갱신 메모리 `project_company_runtime_pattern.md` — 측정 17 → 21항목
- HANDOFF 통찰 4/5 보강 + 측정 21항목 갱신 + 추가 커밋·푸시
...
| 파일 | 변경 |
|---|---|
| `80-Issue/02-Issue-02/00-index-issue-02.md` | #4 상태 `Open` → `**Closed** (2026-04-16, 설계서 + 스킬 반영)` — 파일 헤더와 인덱스 불일치 해소 |
| `80-Issue/00-index.md` | Issue-02 중간 카운트 `2` → `1` |
| `80-Issue/07-Issue-07/00-index-issue-07.md` | 분할 트리거 #1 "섹션 수" 카운트 기준 주석 추가 (번호 섹션만) + 2026-04-21 현재 수치 박제 (15개 / §1 71줄) |

### Open Issue 훑기 결과 (2026-04-21 기준 5건)

| # | 제목 | 상태 요약 |
|---|---|---|
| 02 | 전체 프로젝트 구조 리뷰 | Open 6건 (높음 2 / 중간 1 / 낮음 2) — 파이프라인 실험 / L/XL 파일럿 트리거 |
| 05 | Opus × Gemini Debate | AI-1~4 Done, EXP-1/2/3 실험 대기 (EXP-2는 Issue-02 #3과 데이터 공유) |
| 06 | TDD-ready 설계서 규격 | **v0.2.0 stable**. 배치 엔진 3건 실측 후 v0.3.0 재검토 |
| 07 | common-spec 분할 검토 | 트리거 4건 **전부 미도달** (현재 15개 / §1 71줄, 500줄 여유 큼) |
| 08 | 리스크 매트릭스 | **1차 구현 완료** (#1~#7). 축 B는 Phase 3 배치 엔진 실측 시 자연 활성 |

### 점검 결론

- Issue-02 #4 반영 시점 지연 동기화만 필요 → 수정 3건으로 해소.
- Issue-07 트리거 여전히 미도달. 보류 유지. `##` 전체(24개)가 아닌 번호 섹션(15개) 기준임을 문서에 명시해둠 → 향후 해석 논쟁 방지.

### 다음 세션 (내일 회사) 준비 지시

- **Phase 3 배치 엔진 shackled TDD 실측** — Issue-06 v0.2.0 + Issue-08 축 A/C/E 1차 박제 기반 첫 실전.
  - 실측 전: 배치 엔진 설계서에 대해 wtth 설계 리뷰 선행 (tdd_mode=yes 전제 시 게이트).
  - `tdd_mode` 결정 선행 (inquisition 체급 판정 + 사람 Gate).
  - tdd-ready-spec v0.2.0 MUST 신호 9개 자가 체크.
- **실측 결과 → 환류 트리거**:
  - Issue-06 v0.3.0 MUST/SHOULD 재분류 (HANDOFF 2026-04-20 엔트리 재평가 메모 §참조).
  - Issue-08 축 B 수용률 지표 (`common-spec §13` 파일럿 정량 지표 확장) 실측 데이터 축적.
- **회사 머신 스킬 버전 동기화 체크**: `~/.claude/commands/papillon/{inquisition,wtth}.md` frontmatter `claude-opus-4-7` 여부 (집 머신 2026-04-18 적용 완료).

## 이전 작업: 2026-04-21 (회사) — Issue-08 개설 + 1차 구현 완료
