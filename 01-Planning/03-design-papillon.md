...
사람: /papillon 호출
  ...
  ▼
[진입] 컨텍스트 스캔
  │  CWD에서 산출물 탐색 (00-index.md, docs/decisions/, 00-interview-log.md)
...
| `00-interview-log.md` 존재 | 인터뷰 진행 중 세션 끊김 | 인터뷰 재개 제안 |
| 아무것도 없음 | 이 저장소에서 첫 사용 | Phase 1(인터뷰)부터 시작 |

- 스캔 결과를 사람에게 제시하고 진입점을 **제안**한다. 사람이 승인하면 해당 지점으로 진입하고, 오버라이드하면 사람이 지정한 지점으로 진입한다.
- 이 스캔은 papillon 오케스트레이터의 진입 로직이다. wtth의 "이전 결정 스캔"(Core §3.3)과는 역할이 다르다 — Core §3.3은 리뷰 시작 시 기결정 사항을 전문가��게 전달하는 것이고, 여기는 파이프라인 자체의 진입점을 결정하는 것이다.
...
2. 요구사항의 복잡도를 판별한다:
   - **단순** (명확한 단일 작업, 추가 확인 불필요): 인터뷰 생략 → Step 2(체급 판정)로 직행
   - **복합** (함의/제약 확인이 필요): 아래 구조화 인터뷰 수행
3. (복합) Sonnet이 요구사항을 분석하여 다음을 수행한다:
   - 기술 키워드, 도메인 개념 추출
   - 각 키워드 뒤에 숨겨진 함의(implication)와 암시적 제약 후보 감지
   - 기술적 결정의 의도, 실행 순서 제약, 핵심 전제 조건에 대해 사람에게 확인 질문
4. (복합) 사람이 답변하면, 답변 내용에서 추가 확인이 필요한 부분을 재질문한다.
5. (복합) **매 답변 기록**: 사람의 답변이 올 때마다 확인된 내용을 **인터뷰 기록 파일에 즉시 추가**한다. 이후 질문 시 이 파일을 참조하여 이미 확인된 내용과 모순되지 않도록 한다.
...
7. (복합) 종료 시 인터뷰 기록 파일을 **최종 확정본으로 정리**한다: 확인된 요구사항, 핵심 제약, 암시적 전제 조건 목록. 이 파일이 이후 체급 판정과 Draft 생성의 입력이 된다.
