...
- **Status:**
  - Work: **Stable (v0.2.0, patch 2026-04-21 — AP-3 추가)**
  - Review: None
...
| **v0.2.0 patch** | **2026-04-21** | Issue-08 §축 C 연동 | §5 에 AP-3 "확률 무시 방어" 추가. common-spec §1.6 AP-Severity-1 과 정합. |
...
## 5. Anti-pattern (3건 + v0.2.0 학습 케이스)
...
### AP-3: 확률 무시 방어 RED 금지 (v0.2.0 patch 2026-04-21, Issue-08 §축 C)

- **정의**: RED 가 검증하는 실패 시나리오의 **발생 확률** 을 reasoning 하지 않고, 영향만 근거로 RED 를 추가하거나 심각도 승격. `common-spec §1.6 AP-Severity-1` 의 TDD 맥락 특수판.
- **권고**: 각 RED 에 대해:
  - 시나리오 발생 조건 열거.
  - 각 조건의 bucket 대조 (`baseline-probability.md` §3).
  - 결합 확률 추정.
  - 결합 확률이 `극미` 이면 RED 자체를 TDD 루프에서 제외하고 `Verification` 섹션 또는 운영 관측 (F-NN) 으로 이관.
- **위반 예시**: "Elasticache 45분+ 장애 → 데이터 유실 RED 작성" (장애 빈도 극미. TDD 루프가 아니라 운영 chaos 테스트 영역).
- **허용 예시**: "락 없는 공유 변수 경합 RED" (빈도 매우 높음, TDD 루프 적합).
- **heuristic 힌트**: "장애 시간 N분 이상", "네트워크 완전 단절 N시간", "AZ 전체 불능" 류 키워드. + 해당 인프라의 SLA 교차 확인.
- **판정 성격**: 의미론·도메인 판단 — reviewer 재량. common-spec §1.4 5단계 reasoning 결과와 교차.
