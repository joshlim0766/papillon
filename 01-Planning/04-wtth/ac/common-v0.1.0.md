...
> 페르소나별 특이 사항은 `{persona}-v0.1.0.md`에서 override한다.
...
hard_ac_document:
  required_sections:
    ...
    # 최소 1개 포함해야 한다. 헤딩 문구는 페르소나 AC에서 override 가능.
    - heading: "Findings"
      level: 2
      required: true
      alternatives: ["검토 결과", "리뷰 결과", "지적 사항"]
      # alternatives 중 하나가 존재하면 required 충족
  ...
  invariants:
    - id: "INV-WREV-01"
      rule: "Findings 섹션(또는 alternatives) 1개 이상 존재"
      check: |
        headings = extract_headings(output)
        alternatives = ["Findings", "검토 결과", "리뷰 결과", "지적 사항"]
        any(normalize(h) in [normalize(a) for a in alternatives] for h in headings)
