...
빠삐용이 박제 중인 논점을 **빠삐용 박제 시스템 자체가 실증으로 겪는 케이스 누적**.
...
**측정 17항목** (1차 박제 5축 + 검토 부하 4축 + 변형 C 4축 + 팀원 모방 4축 가능 시):
...
**시범 작성**: 잔여 4건 중 1건 이상을 변형 C 단일 스펙 라이프사이클 (b) 인덱스+분할 형태로 시도. 통찰 2 분할 구조 적용. 측정 17항목 동시 수행.
...
- 갱신 메모리 `project_company_runtime_pattern.md` — 변형 C 한정 조건 + 측정 17항목 + 팀원 자발 모방 추가 표본
