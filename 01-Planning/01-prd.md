...
- **review-hell**: 멀티에이전트 리뷰 시스템. 5명의 도메인 전문가(BE/FE/SRE/SEC/PO)를 독립 에이전트로 병렬 실행하고, 오케스트레이터(Agent S)가 결과를 수집·병합하여 사용자와 항목별 의사결정을 진행한다.
...
| P5 | **코드 리뷰 미지원** — review-hell이 설계 문서 리뷰에 특화. 코드 diff 리뷰 시 전문가 관점이 다름 | 코드 리뷰는 별도 수단 필요 |
| P6 | **PRD 리뷰 미지원** — 대규모 작업의 PRD도 리뷰가 필요하나 현재 review-hell은 설계/코드만 대상 | PRD 품질을 사람이 직접 한 줄씩 검증해야 함 |
...
| G4 | PRD/설계/ADR/운영절차/코드 리뷰를 동일 시스템으로 처리 | review-hell 하나로 5종 리뷰 수행. 각 모드별 전문가 풀과 의사결정 방식은 다름 |
...
| 산출물 | 파일명 | 역할 |
|---|---|---|
| **review-hell v2** | `review-hell.md` | 멀티에이전트 리뷰 엔진. PRD/설계/코드 3모드 지원. RDR/ADR 생성. 수렴 메커니즘 내장 |
| **work-flow** | `work-flow.md` | 파이프라인 오케스트레이터. 인터뷰 → 체급 판정 → Phase 자동 전환. 사람은 의사결정만 |

- 두 스킬 모두 `~/.claude/commands/`에 글로벌 설치
- review-hell은 work-flow 없이 단독 실행 가능
...
| R5 | review-hell 단독 실행 가능 유지 | work-flow 없이도 직접 호출할 수 있어야 함 |
...
1. `review-hell.md` — 개선된 리뷰 엔진 스킬
2. `work-flow.md` — 파이프라인 오케스트레이터 스킬
