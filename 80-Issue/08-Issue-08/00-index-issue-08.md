...
## 상태
- **Open (1차 구현 완료)** — 2026-04-21 개설 + 당일 #1~#7 박제 완료. 실측 피드백 대기 상태로 전환.
...
| # | 작업 | 산출물 | 의존 | 상태 |
|---|---|---|---|---|
| 1+2 | **확률 bucket 어휘 + prior 카탈로그 초안** (통합) | `01-Planning/baseline-probability.md` v0.1 | 독립 | **완료** (2026-04-21) |
| 3 | `common-spec §1` 심각도 기준 확장 — 영향 × 확률 매트릭스 박제 | `02-common-spec.md` 갱신 (`baseline-probability.md` §2 매트릭스 인용) | #1+2 | **완료** (2026-04-21) |
| 4 | `04-wtth` reviewer 페르소나 프롬프트 "확률 reasoning" 지침 추가 (축 A) | 설계 SSOT + 스킬 프롬프트 갱신 | #1+2, #3 | **완료** (2026-04-21) |
| 5 | anti-pattern "확률 무시 방어 금지" 신설 (축 C) | common-spec §1.6 + TDD-ready spec v0.2.0 patch (AP-3) | #3 | **완료** (2026-04-21) |
| 6 | RDR 포맷 확장 — 조건/확률 reasoning 박제 (축 B) | common-spec §2 RDR 표 + Finding 상세 섹션 템플릿 | #3 | **완료** (2026-04-21) |
| 7 | reviewer AC 에 확률 축 invariant 추가 | common-v0.1.2.md 신규 (INV-WREV-06) | #4 | **완료** (2026-04-21, common 베이스만. 페르소나 override 필요 시 후속) |

**진행 상황 (2026-04-21 회사)**: **#1~#7 전부 완료**. 축 A/B/C/E 1차 박제 종료. Phase 3 배치 엔진 착수 시 실측 → 축 B (수용률 캘리브레이션) 자연 활성.

**후속 (v0.2.0 추가 보강 후보)**:
- 페르소나별 AC (`be`, `fe`, `test`, `sec`, `arch`, `dba`, `code`) 에 확률 축 override 필요 여부 — 실측 후 결정. 현재 common-v0.1.2 베이스만으로 대부분 커버.
- SEM-WREV-05 (확률 reasoning 타당성) Tier 2 승격 — HITL escalation 3회 누적 시.
- baseline-probability.md 내부 실측 prior 보강 (분기 메타리뷰).
