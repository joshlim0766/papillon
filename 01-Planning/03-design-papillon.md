...
3. 초안을 사람에게 보여주고 Phase 2(설계 리뷰)로 자동 전환한다.
...
**4-A. 개발 작업 (코드)**
1. 오케스트레이터가 shackled을 호출하여 확정된 설계서의 태스크 목록을 순서대로 구현한다.
2. Phase 5로 자동 전환.

**4-B. 인프라/수행 작업**
1. 오케스트레이터가 shackled을 호출하여 태스크 내용을 기반으로 **런북(절차서)을 생성**한다. 런북은 [50-runbook-template.md](./50-runbook-template.md)의 형식을 따른다.
2. 오케스트레이터가 wtth을 **운영 절차 리뷰 모드**로 호출한다.
3. ★ 사람: finding별 수용/기각.
4. 오케스트레이터가 shackled을 호출하여 절차서 확정 후, 각 단계를 순서대로 실행한다.
5. 각 단계의 검증 커맨드를 수행하여 성공 여부를 판단한다.
6. 전체 완료 확인 후:
   - 커밋 대상이 있는 경우 (런북, 설정 파일, RDR 등) → Phase 6으로 전환
   - 커밋 대상이 없는 경우 → RDR 생성 후 파이프라인 종료

**진행 불가 시:** shackled이 구현 중 진행 불가 상황에 도달하면 원인 힌트를 제공하고, 사람이 롤백 지점을 결정한다.
