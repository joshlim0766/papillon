...
**박제 회피 원칙**: shackled 실측 데이터(2E·2F 특히)는 현재 진행 중이므로 이 메모는 **MUST/SHOULD 재분류를 확정하지 않는다**. 구조적 신호 충족 여부만 기록하고, "이 신호가 없었다면 shackled가 실패했을 곳" 판정은 실측 후로 미룬다.
...
- [ ] shackled 실측 데이터 수집 — 2e/2f TDD 진행 결과 (P0/P1 건수, TDD 루프 이탈 여부, 실패 지점)
...
- megabird 레포는 현재 phase 2e 진행 중이므로 원본 파일 수정·복제 금지 유지.
