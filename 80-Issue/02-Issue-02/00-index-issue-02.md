...
| 시점 | 이슈 | 솔루션 핵심 |
|---|---|---|
| **지금 바로** | #1 설계 vs 프롬프트 | 새 문서부터 `[SPEC]`/`[RAT]` 마킹 적용 |
| **지금 바로** | #3 사람 개입 과다 | 확인 지점 3등급 분류 (Gate/Checkpoint/FYI) |
| **묶음 B 설계 시** | #4 wtth 모드별 상세 | 모드별 스킵/변형/추가 규칙 명시 |
| **파이프라인 1~2회 후** | #2 컨텍스트 윈도우 | Phase별 토큰 버짓 + 컨텍스트 리셋 포인트 |
| **파이프라인 1~2회 후** | #6 체급 heuristic | 캘리브레이션 사례 축적 |
| **파이프라인 1~2회 후** | #5 산출물 생명주기 | 생명주기 매트릭스 (common-spec에 추가) |
