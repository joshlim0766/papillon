...
- [ ] **2e 실측 데이터 수집** — shackled TDD 실행 + 코드 구현 완료 상태. RED 1~9 결과, TDD 루프 이탈 여부, 지시서 공백 발생 지점, 구현 중 예상 외 문제, wtth 리뷰 대비 실측 차이. 수집 경로: shackled RDR / 구현 커밋 메시지 / TDD 로그 (위치는 주인님 확인 필요)
- [ ] **2f 실측 데이터 수집** — shackled TDD 진행 완료 후 동일 축으로 수집
- [ ] 환류 §8 tracing 4건 실측 적용 — 2e 데이터로 부분 수행 가능, 2f 완료 후 종합
