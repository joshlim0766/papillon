...
| 5 | anti-pattern "확률 무시 방어 금지" 신설 (축 C) | `tdd-ready-spec v0.2.1` + common-spec 반영 | #3 | 대기 |
| 6 | RDR 포맷 확장 — 조건/확률 reasoning 박제 (축 B) | `common-spec §2` RDR 포맷 확장 | #3 | 대기 |
| 7 | reviewer AC 에 확률 축 invariant 추가 | `01-Planning/04-wtth/ac/` 7 페르소나 전부 | #4 | 대기 |

**진행 상황**: #1+2 통합 완료 (VOCABULARY.yaml 은 keyword extraction 용이라 별도 박제 선택). 다음 #3 → #4 → 병렬(#5/#6/#7).

**우선순위**: Phase 3 배치 엔진 착수 전 최소 **#4** (축 A reviewer 프롬프트) 까지 완료 권장.
