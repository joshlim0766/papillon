...
**박제 회피 원칙**: 2e는 shackled TDD 실행 + 코드 구현 완료 → **2e 실측 데이터는 존재**. 2f는 진행 전. 이 메모는 10개 exemplar의 **구조적 신호 충족 여부**만 박제하고, "이 신호가 없었다면 shackled가 실패했을 곳" tracing은 2e 기준으로 우선 수행 가능 (별도 문서). 2f 데이터 추가 후 전체 v0.2.0 MUST/SHOULD 재분류 확정.
...
- [x] **2e 실측 데이터 수집** — `tracing-2e-2026-04-20.md` 에 박제 완료 (2026-04-20). RED 1~9 전부 GREEN, 지시서 공백 3건 (§2.4 UNIQUE KEY 오탈자 / §4.4 단서조항 / TenantContextMismatchException 미열거) 식별.
- [x] **2f 실측 데이터 수집** — `tracing-2f-2026-04-20.md` 1차 완료. 관찰 4건 박제 (1 시간축 / 2 공간축 / 3 3R 재귀 패턴 / 4 수렴 + 실용 노선 + AB-2 신호 #9 실증). 2f 4라운드 수렴 103/103 GREEN, 실용 노선으로 AB-1/4/5 운영 피드백 후로 위임.
- [ ] 환류 §8 tracing 4건 정식 적용 — 2e + 2f 1차 완료. §8 "최소 3건" 점검 필요 (2a~2d 과거 데이터 tracing 추가 여부).
- [ ] **v0.2.0 draft 갱신** — 신호 #8 확장(권위 소스 교차) / #2 확장(예외 타입 카탈로그) / #7B 단서조항 / **#9 신설(실패 경로 계약)** + 실패 계약 표 템플릿 + 지시서 분할 요건. 체계 개선 항목 2건(품질 경계 4지표 / 실용 노선 휴리스틱) 포함.
